...
SDL_Surface* SDL_ScaleSurface (SDL_Surface* s, Uint16 w, Uint16 h)
{
    ...
    SDL_Surface *resize = SDL_CreateRGBSurface(s->flags, w, h, s->format->BitsPerPixel, s->format->Rmask, s->format->Gmask, s->format->Bmask, s->format->Amask);
    
    double stretch_x = ((double)(w) / (double)(s->w));
    double stretch_y = ((double)(h) / (double)(s->h));

    for( Sint32 y = 0; y < s->h; y++)
    {
        for( Sint32 x = 0; x < s->w; x++)
        {
            for( Sint32 o_y = 0; o_y < stretch_y; o_y ++)
            {
                for( Sint32 o_x = 0; o_x < stretch_x; o_x ++)
                {
                    putpixel( resize, (Sint32)(stretch_x * x) + o_x, (Sint32)(stretch_y * y) + o_y, getpixel (s,x,y));
                }
            }
        }
    }
...
char* create_list_matrix( SDL_Surface* s)
{
    ...
    for ( int i = 0; i < 16; i ++)
    {
        for ( int j = 0; j < 16; j++)
        {
            ...
            Uint8 red = 0;
            SDL_GetRGB(pixel, s->format, &red, &red, &red);
            if (red == 0) //pixel noir detecte -> texte
            {
                *(m + i + (j * 16)) = 1;
            }
