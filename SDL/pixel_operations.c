...
SDL_Surface* sub_image(SDL_Surface* image, size_t xmin, size_t xmax, size_t ymin, size_t ymax)
{
	SDL_Surface* sub_image = SDL_CreateRGBSurface(image->flags, xmax - xmin, ymax - ymin, image->format->BitsPerPixel, image->format->Rmask, image->format->Gmask, image->format->Bmask, image->format->Amask);
	...
	for(size_t j = 0; j < height; j++)
	{
		...
		{
			Uint32 pixel = getpixel(image, xmin + i, ymin +  j);			
			putpixel(sub_image,i,j, pixel);
		}
	}
	return sub_image;
}
